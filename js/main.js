...
import { initDualAxis, updateDualAxis, computeInsight as computeDualAxisInsight } from './charts/dualaxis.js';
import { initSlopeChart, updateSlopeChart, computeSection3Insight } from './charts/slopechart.js';
import { initScatter, updateScatter } from './charts/scatter.js';
import { initStackedArea, updateStackedArea, renderSmallMultiples, computeSection4Insight } from './charts/stackedarea.js';
...
function populateKPIs(data) {
  ...
  const totalBreathLatest = d3.sum(latestYearRows, d => d.breath_test_conducted || 0);
  const totalBreathPrev = d3.sum(prevYearRows, d => d.breath_test_conducted || 0);
  ...
  const latestBreathPos = breathTests.filter(d => d.YEAR === maxYear && d.AGE_GROUP === 'all ages');
  const firstYearBreathPos = breathTests.filter(d => d.YEAR === minYear && d.AGE_GROUP === 'all ages');
  ...
  for (const row of latestBreathPos) {
    ...
    if (testRow && testRow.breath_test_conducted > 0 && row.positive_breath_tests != null) {
      latestRates.push(row.positive_breath_tests / testRow.breath_test_conducted * 100);
    }
  }
  ...
  for (const row of firstYearBreathPos) {
    ...
    if (testRow && testRow.breath_test_conducted > 0 && row.positive_breath_tests != null) {
      firstRates.push(row.positive_breath_tests / testRow.breath_test_conducted * 100);
    }
  }
  ...
  const rateDirection = rateDiff < 0 ? 'improving' : rateDiff > 0 ? 'worsening' : 'stable';
  const rateTrendClass = rateDiff < 0 ? 'positive' : rateDiff > 0 ? 'negative' : 'neutral';

  setKPI(2,
    'Avg positivity rate',
    d3.format('.2f')(avgRateLatest) + ' %',
    { text: `${rateDiff < 0 ? '\u2193' : '\u2191'} ${rateDirection} since ${minYear}`, cls: rateTrendClass }
  );

  // ── KPI 3: Total drug tests in most recent year ──
  const totalDrugLatest = d3.sum(latestYearRows, d => d.drug_test_conducted || 0);
  ...
  if (totalDrugLatest > 0) {
    ...
    setKPI(3,
      `Total tests (${maxYear})`,
      d3.format('.2s')(totalAll),
      { text: 'all jurisdictions', cls: 'neutral' }
    );
  }
  ...
  setKPI(4,
    ...
    d3.format('.2s')(totalFines),
    { text: `${finesMinYear}\u2013${finesMaxYear}`, cls: 'neutral' }
  );
}
...
function buildFilterBar(sectionId, dataset) {
  ...
  // Derive year range
  const years = dataset.map(d => d.YEAR).filter(y => y != null && !isNaN(y));
...
const POPULATIONS = {
  nsw: 8200000, vic: 6600000, qld: 5200000, wa: 2700000,
  sa: 1800000, tas: 560000, act: 460000, nt: 250000,
};

function buildSection01Insight(data, yearRange) {
  // Calculate tests per capita for each jurisdiction in latest year
  const latestYear = yearRange ? yearRange[1] : Math.max(...data.map(d => d.YEAR).filter(y => y != null));
  const rows = data.filter(d => d.YEAR === latestYear);
  const perCapita = rows.map(d => ({
    j: d.JURISDICTION,
...
async function init() {
  try {
    ...
    console.log('%c All datasets loaded successfully', 'color: #2E8B57; font-weight: bold;');
    ...
    buildFilterBar('section-01', data.alcoholDrug);
    buildFilterBar('section-02', data.alcoholDrug);
    buildFilterBar('section-03', data.breathTests);
    buildFilterBar('section-04', data.fines);
    ...
    if (insight01) insight01.innerHTML = buildSection01Insight(data.alcoholDrug);
    ...
    if (insight02) {
      const insightText = computeDualAxisInsight(data.alcoholDrug, data.breathTests);
    ...
    if (insight03) {
      insight03.innerHTML = computeSection3Insight(data.alcoholDrug, data.breathTests);
    }
    ...
    if (footerSource) {
      const allYears = data.alcoholDrug.map(d => d.YEAR).filter(y => y != null);
    ...
    document.addEventListener('filterchange', (e) => {
      const { sectionId, states, yearRange: yr } = e.detail;
      ...
      if (sectionId === 'section-01') {
        ...
        if (insight01) insight01.innerHTML = buildSection01Insight(data.alcoholDrug, yr);
      }
