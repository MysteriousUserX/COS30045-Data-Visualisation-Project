...
function aggregateByState(data, yr) {
  ...
  for (const d of data) {
    ...
    map.set(j, map.get(j) + (d.breath_test_conducted || 0) + (d.drug_test_conducted || 0));
  }
  return map;
}
...
export function initChoropleth(selector, geoJSON, csvData) {
  ...
  projection.fitExtent(
    [[pad, pad], [width - pad, height - pad - 36]],
    geoJSON
  );
  ...
  g.selectAll('path.state')
    ...
    .on('mouseenter', function (event, d) {
      ...
      const name = d.properties.STATE_NAME || d._jurisdiction?.toUpperCase() || 'Unknown';
      ...
      tooltip.show(
        `<b style="font-size:13px">${name}</b><br>
         ...
         <span style="color:#9A9A9A;font-size:11px">${yearRange[0]}\u2013${yearRange[1]}</span>`,
        event
      );
    })
  ...
  if (window.ResizeObserver) {
    const ro = new ResizeObserver(() => {
      ...
      projection.fitExtent([[pad, pad], [w - pad, h - pad - 36]], geoJSON);
...
function applySelectionStrokes() {
  g.selectAll('path.state')
    .attr('stroke', d => selectedStates.has(d._jurisdiction) ? '#1B6CC4' : 'rgba(255,255,255,0.25)')
    .attr('stroke-width', d => selectedStates.has(d._jurisdiction) ? 2.5 : 0.5);
}
...
function buildLegend(svg, width, height, maxVal) {
  ...
  // Labels
  lg.append('text').attr('x', 0).attr('y', lh + 12)
    .attr('text-anchor', 'start')
    .style('font-size', '9px').style('fill', 'rgba(255,255,255,0.4)')
    .style('font-family', "'Inter', sans-serif")
    .text('Low');

  lg.append('text').attr('x', lw).attr('y', lh + 12)
...
// ─── Update ───
export function updateChoropleth(filteredData, newYearRange, newSelectedStates) {
