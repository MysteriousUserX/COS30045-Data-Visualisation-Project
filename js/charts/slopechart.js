...
function getPositiveTests(breathTests, j, yr) {
  const a = breathTests.find(d => d.JURISDICTION === j && d.YEAR === yr && d.AGE_GROUP === 'all ages');
...
function calcRate(ad, bt, j, yr) {
  ...
  if (!t || p == null || !t.breath_test_conducted || t.breath_test_conducted <= 0) return null;
...
export function initSlopeChart(selector, alcoholDrug, breathTests) {
  ...
  _allYears = [...new Set(alcoholDrug.map(d => d.YEAR).filter(y => y != null))].sort((a, b) => a - b);
  if (_allYears.length < 2) { _container.text('Insufficient data'); return; }
  ...
  _selectA = ctrls.append('select').attr('id', 'slope-year-a').attr('class', 'slope-select');
  _allYears.forEach(y => _selectA.append('option').attr('value', y).text(y).property('selected', y === _yearA));
  ctrls.append('span').attr('class', 'slope-arrow').html('&#8594;');
  ctrls.append('label').attr('class', 'slope-label').text('Year B');
  _selectB = ctrls.append('select').attr('id', 'slope-year-b').attr('class', 'slope-select');
  _allYears.forEach(y => _selectB.append('option').attr('value', y).text(y).property('selected', y === _yearB));

  _selectA.on('change', () => { _yearA = +_selectA.property('value'); render(true); });
  _selectB.on('change', () => { _yearB = +_selectB.property('value'); render(true); });
  ...
  _g = _svg.append('g').attr('transform', `translate(${_margin.left},${_margin.top})`);
  render(false);
}
...
function render(animate) {
  ...
  let jurisdictions = [...new Set(_alcoholDrug.map(d => d.JURISDICTION).filter(Boolean))];
  if (_activeStates.length > 0) jurisdictions = jurisdictions.filter(j => _activeStates.includes(j));
  ...
  if (!items.length) {
    _g.append('text').attr('x', _width / 2).attr('y', _height / 2)
      .attr('text-anchor', 'middle').attr('fill', '#9A9A9A').attr('font-size', '14px')
      .text('No matching data for selected years');
    return;
  }
  ...
  const xA = 0, xB = _width;

  // Subtle gradient background
  const grad = _svg.select('defs').size() ? _svg.select('defs') : _svg.append('defs');
  if (!grad.select('#slope-bg').size()) {
    const lg = grad.append('linearGradient').attr('id', 'slope-bg').attr('x1', 0).attr('x2', 1).attr('y1', 0).attr('y2', 0);
  ...
  _g.append('rect').attr('x', -10).attr('y', -10).attr('width', _width + 20).attr('height', _height + 20)
    .attr('rx', 8).attr('fill', 'url(#slope-bg)').attr('opacity', 0.6);

  // Horizontal grid
  y.ticks(5).forEach(t => {
    _g.append('line').attr('x1', xA).attr('x2', xB).attr('y1', y(t)).attr('y2', y(t))
      .attr('stroke', '#E8E6E3').attr('stroke-width', 0.5).attr('stroke-dasharray', '3,3');
    _g.append('text').attr('x', -8).attr('y', y(t)).attr('text-anchor', 'end').attr('dominant-baseline', 'middle')
      .attr('fill', '#B0AEAB').attr('font-size', '9px').text(t.toFixed(1) + '%');
  });

  // Column axis lines
  _g.append('line').attr('x1', xA).attr('x2', xA).attr('y1', -5).attr('y2', _height + 5)
    .attr('stroke', '#D5D3D0').attr('stroke-width', 1);
  _g.append('line').attr('x1', xB).attr('x2', xB).attr('y1', -5).attr('y2', _height + 5)
    .attr('stroke', '#D5D3D0').attr('stroke-width', 1);

  // Column titles
  _g.append('text').attr('x', xA).attr('y', -22).attr('text-anchor', 'middle')
    .attr('fill', '#1A1A1A').attr('font-size', '15px').attr('font-weight', 700).text(_yearA);
  _g.append('text').attr('x', xB).attr('y', -22).attr('text-anchor', 'middle')
    .attr('fill', '#1A1A1A').attr('font-size', '15px').attr('font-weight', 700).text(_yearB);

  // Sort by biggest absolute change for emphasis
  const sorted = [...items].sort((a, b) => a.change - b.change);
  const topImp = sorted[0], topWor = sorted[sorted.length - 1];
  ...
  for (const d of items) {
    ...
    // Connecting line
    const line = _g.append('line')
      .attr('x1', xA).attr('y1', yA)
      .attr('x2', animate ? xA : xB).attr('y2', animate ? yA : yB)
      .attr('stroke', color).attr('stroke-width', lw).attr('opacity', opacity);
    if (Math.abs(ch) < 0.1) line.attr('stroke-dasharray', '5,4');
    if (animate) line.transition().duration(400).attr('x2', xB).attr('y2', yB);

    // Hover
    _g.append('line').attr('x1', xA).attr('y1', yA).attr('x2', xB).attr('y2', yB)
      ...
      .on('mouseover', (ev) => {
        ...
        const sign = ch >= 0 ? '+' : '';
        tooltip.show(`<strong>${d.j.toUpperCase()}</strong><br>${_yearA}: ${d.rA.toFixed(2)}% &rarr; ${_yearB}: ${d.rB.toFixed(2)}%<br>Change: ${sign}${ch.toFixed(2)}%`, ev);
      })
    ...
    [{ cx: xA, cy: yA }, { cx: xB, cy: yB }].forEach(p => {
      _g.append('circle').attr('class', 'slope-dot-' + d.j)
        .attr('cx', p.cx).attr('cy', p.cy).attr('r', 5)
    ...
    const fs = emphasis ? '12px' : '11px';

    _g.append('text').attr('x', xA - 14).attr('y', lL.y)
      .attr('text-anchor', 'end').attr('dominant-baseline', 'middle')
      .attr('fill', labelColor).attr('font-size', fs).attr('font-weight', fw)
      .text(`${d.j.toUpperCase()} ${d.rA.toFixed(2)}%`);

    // Connector from label to dot if label was pushed
    if (Math.abs(lL.y - yA) > 4) {
      _g.append('line').attr('x1', xA - 12).attr('y1', lL.y).attr('x2', xA - 2).attr('y2', yA)
        .attr('stroke', '#D5D3D0').attr('stroke-width', 0.5);
    }

    _g.append('text').attr('x', xB + 14).attr('y', lR.y)
      .attr('text-anchor', 'start').attr('dominant-baseline', 'middle')
      .attr('fill', labelColor).attr('font-size', fs).attr('font-weight', fw)
      .text(`${d.rB.toFixed(2)}% ${d.j.toUpperCase()}`);

    if (Math.abs(lR.y - yB) > 4) {
      _g.append('line').attr('x1', xB + 12).attr('y1', lR.y).attr('x2', xB + 2).attr('y2', yB)
        .attr('stroke', '#D5D3D0').attr('stroke-width', 0.5);
    }
  }
}
...
export function updateSlopeChart(alcoholDrug, breathTests, yearRange, states) {
  ...
  if (yearRange) {
    ...
    [_selectA, _selectB].forEach((sel, i) => {
      ...
      fy.forEach(y => sel.append('option').attr('value', y).text(y).property('selected', y === (i === 0 ? _yearA : _yearB)));
    });
  }
  render(true);
}
...
export function computeSection3Insight(alcoholDrug, breathTests) {
  const years = [...new Set(alcoholDrug.map(d => d.YEAR).filter(y => y != null))].sort((a, b) => a - b);
  if (years.length < 2) return '';
  const first = years[0], last = years[years.length - 1];
  const jurisdictions = [...new Set(alcoholDrug.map(d => d.JURISDICTION).filter(Boolean))];
  const changes = [];
  for (const j of jurisdictions) {
