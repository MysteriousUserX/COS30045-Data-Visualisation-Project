...
function joinData(alcoholDrug, breathTests, states, yr, mode) {
  ...
  return filtered.map(year => {
    const adRows = alcoholDrug.filter(d =>
      d.YEAR === year && (states.length === 0 || states.includes(d.JURISDICTION))
    );
    const btRows = breathTests.filter(d =>
      d.YEAR === year &&
...
function hasDrugData(alcoholDrug) {
  const withDrug = alcoholDrug.filter(d => d.drug_test_conducted != null && d.drug_test_conducted > 0);
...
export function initDualAxis(selector, alcoholDrug, breathTests) {
  ...
  if (hasDrugData(alcoholDrug)) {
    ...
    toggleWrap.innerHTML = `
      <button class="toggle-btn active" data-mode="alcohol">Alcohol</button>
      <button class="toggle-btn" data-mode="drug">Drug</button>
    `;
...
function renderChart(alcoholDrug, breathTests) {
  ...
  const joined = joinData(alcoholDrug, breathTests, selectedStates, yearRange, currentMode);
  ...
  // Axes
  gXAxis.transition().duration(400)
    .call(d3.axisBottom(xScale).tickValues(
      joined.filter((_, i) => i % Math.ceil(joined.length / 12) === 0).map(d => d.year)
    ))
    .call(g => g.select('.domain').attr('stroke', '#E0DEDA'))
    .selectAll('text').style('font-size', '10px').style('fill', '#9A9A9A');

  gYLeft.transition().duration(400)
    .call(d3.axisLeft(yLeft).ticks(6).tickFormat(d3.format('.2s')))
    .call(g => g.select('.domain').remove())
    .call(g => g.selectAll('.tick line').attr('stroke', '#E0DEDA').attr('x2', width).attr('opacity', 0.4))
    .selectAll('text').style('font-size', '10px').style('fill', '#9A9A9A');

  gYRight.transition().duration(400)
    .call(d3.axisRight(yRight).ticks(5).tickFormat(d => d + '%'))
    .call(g => g.select('.domain').remove())
    .call(g => g.selectAll('.tick line').remove())
    .selectAll('text').style('font-size', '10px').style('fill', '#E09420');

  // Update left label
  svg.select('.y-left-label')
    .text(currentMode === 'drug' ? 'Drug tests conducted' : 'Breath tests conducted');
  ...
  bars.join(
    enter => enter.append('rect')
      ...
      .call(enter => enter.transition().duration(600).ease(d3.easeQuadOut)
  ...
  linePath.join(
    ...
    update => update.transition().duration(400).attr('d', lineGen)
  );
  ...
  dots.join(
    enter => enter.append('circle')
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip.show(`<b>${d.year}</b><br>Positivity rate: ${d3.format('.2f')(d.rate)}%`, event);
      })
      .on('mouseout', function () {
  ...
  annotations.forEach(ann => {
    ...
    // Label bg
    const labelG = gAnnotations.append('g')
...
// ─── Update (from filter) ───
export function updateDualAxis(alcoholDrug, breathTests, newYearRange, newSelectedStates) {
...
export function computeInsight(alcoholDrug, breathTests) {
  ...
  let correlation = '';
  if (testsUp && rateDown) {
    correlation = 'As testing volumes increased over the period, positivity rates generally declined — suggesting a possible deterrent effect.';
  } else if (testsUp && !rateDown) {
    correlation = 'Despite increased testing volumes, positivity rates have not declined — raising questions about enforcement effectiveness.';
  } else {
    correlation = 'Both testing volumes and positivity rates have shifted over the period.';
  }
