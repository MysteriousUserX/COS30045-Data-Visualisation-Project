...
function aggregate(data, yr) {
  ...
  for (const d of data) {
    ...
    if (!map.has(j)) {
      map.set(j, { jurisdiction: j, breath_test_conducted: 0, drug_test_conducted: 0 });
    }
  ...
  return Array.from(map.values())
    .map(e => ({ ...e, total: e.breath_test_conducted + e.drug_test_conducted }))
...
export function initBarChart(selector, data) {
  ...
  title.style.cssText = 'font-size:16px;font-weight:600;color:#1A1A1A;margin-bottom:20px;';
...
function drawBars(agg, animate) {
  ...
  agg.forEach((row, i) => {
    ...
    testColumns.forEach(col => {
      ...
      if (animate) {
        gBars.select(`rect[data-jurisdiction="${row.jurisdiction}"][data-column="${col}"]`)
          .transition()
          .delay(i * 60)
          .duration(600)
          .ease(d3.easeQuadOut)
...
export function updateBarChart(filteredData, newYearRange, newHighlightStates) {
  ...
  // Update y-axis
  gYAxis.transition().duration(400)
    .call(d3.axisLeft(yScale).tickSize(0))
    .call(g => g.select('.domain').remove());
